...
func (gl *Client) Write(p []byte) (int, error) {
	...
	zipped := buf.Bytes() // HLreview

	count, rem := len(zipped)/maxChunkSize, len(zipped)%maxChunkSize
	if rem > 0 {
		count++
	}

	if count > maxChunkCount {
		return 0, fmt.Errorf("message exceeds maximum size, %d > %d", len(zipped), maxChunkCount*maxChunkSize)
	}
	...
	id := gl.messageID()
	for i := 0; i < count; i++ {
		packet := append([]byte{}, gelfMagicByteA, gelfMagicByteB) // magic GELF bytes // HLreview
		packet = append(packet, id[0:8]...)
		packet = append(packet, uint8(i), uint8(count)) // sequence

		chunkSize := maxChunkSize
		if len(zipped) < chunkSize {
			chunkSize = len(zipped)
		}
		packet, zipped = append(packet, zipped[:chunkSize]...), zipped[chunkSize:] // HLreview
